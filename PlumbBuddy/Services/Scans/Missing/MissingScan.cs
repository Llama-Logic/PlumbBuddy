        ...
        else
        {
            ...
            if (RequiredPackageKeys is { } keys)
                foreach (var key in keys)
                {
                    ...
                    if (!await pbDbContext.ModFileResources.AnyAsync(mfr => mfr.KeyType == signedType && mfr.KeyGroup == signedGroup && mfr.KeyFullInstance == signedFullInstance && mfr.ModFileHash.ModFiles!.Any(mf => mf.FoundAbsent == null)).ConfigureAwait(false))
                    {
            ...
            if (!requiredElementMissing && RequiredScriptArchiveFullNames is { } fullNames)
                ...
                {
                    if (!await pbDbContext.ScriptModArchiveEntries.AnyAsync(smae => smae.FullName == fullName && smae.ModFileHash.ModFiles.Any(mf => mf.FoundAbsent == null)).ConfigureAwait(false))
                    {
                        requiredElementMissing = true;
                        break;
                    }
